/* ============================================================
   ...
   // Deslocamento vertical entre andares (no espaço 320x145 do sprite).
   const STACK_OFFSET = 60;
   // Estado da rotação 3D do preview.
   let rotX = 0, rotY = 0;
   ...
   function atualizarPreview() {
       ...
       const cards = [...document.querySelectorAll('.bloco-card')]; // Bloco 1 = base ... N = topo
       const N = cards.length || 1;
       const totalH = 145 + (N - 1) * STACK_OFFSET;
   
       const stage = document.querySelector('#produtoStage');
       stage.style.aspectRatio = `320 / ${totalH}`;
   
       // (Re)cria o rotor que recebe a rotação, preservando o ângulo atual.
       let rotor = document.querySelector('#produtoRotor');
       if (!rotor) {
           rotor = document.createElement('div');
           rotor.id = 'produtoRotor';
           rotor.className = 'produto-rotor';
           stage.appendChild(rotor);
       }
       rotor.innerHTML = '';
   
       const hPct = (145 / totalH) * 100;
       const resumo = [];
   
       cards.forEach((card, idx) => {
           const ordem = idx + 1;                          // 1 (base) .. N (topo)
           const topPct = ((N - ordem) * STACK_OFFSET / totalH) * 100;
   
           const andar = document.createElement('div');
           andar.className = 'pv-andar';
           andar.style.top = topPct + '%';
           andar.style.height = hPct + '%';
           andar.style.zIndex = ordem;                     // andar de cima por cima
   
           const corBloco = Number(card.querySelector('.corBloco').value || 0);
           andar.appendChild(camada(imgBloco(corBloco), 1));
           ...
           card.querySelectorAll('.lamina-row').forEach(row => {
               ...
               if (pos >= 1 && pos <= 3 && cor > 0) {
                   andar.appendChild(camada(imgLamina(pos, cor), 2 + pos));
                   if (pad > 0) andar.appendChild(camada(imgPadrao(pad, pos), 6 + pos));
                   descL.push(`${POSICAO_NOME[pos]}: ${LAMINA[cor].nome}${pad ? ' · ' + PADRAO_NOME[pad] : ''}`);
               }
           });
           resumo.push(`Bloco ${ordem} (${TAMPA[corBloco].nome}): ${descL.length ? descL.join(', ') : 'sem lâminas'}`);
           rotor.appendChild(andar);
       });
   
       // Tampa sobre o bloco do topo.
       if (corTampa > 0) {
           const topo = document.createElement('div');
           topo.className = 'pv-andar pv-tampa';
           topo.style.top = '0%';
           topo.style.height = hPct + '%';
           topo.style.zIndex = 50;
           topo.appendChild(camada(imgTampa(corTampa), 1));
           rotor.appendChild(topo);
       }
       ...
       document.querySelector('#previewMeta').innerHTML = `
           ...
           <div>${resumo.map(s => '• ' + s).join('<br>')}</div>`;
   }
   
   /* ---------- Rotação 3D do preview (arrastar para girar) ---------- */
   function aplicarRotacao() {
       const rotor = document.querySelector('#produtoRotor');
       if (rotor) rotor.style.transform = `rotateX(${rotX}deg) rotateY(${rotY}deg)`;
   }
   
   function initRotacao() {
       const stage = document.querySelector('#produtoStage');
       if (!stage) return;
       let girando = false, px = 0, py = 0;
       stage.addEventListener('pointerdown', e => { girando = true; px = e.clientX; py = e.clientY; stage.setPointerCapture(e.pointerId); });
       stage.addEventListener('pointermove', e => {
           if (!girando) return;
           rotY += (e.clientX - px) * 0.6;
           rotX -= (e.clientY - py) * 0.6;
       ...
       stage.addEventListener('dblclick', () => { rotX = 0; rotY = 0; aplicarRotacao(); }); // duplo clique = reset
   }
